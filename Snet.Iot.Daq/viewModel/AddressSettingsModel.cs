        ...
        private async Task DeleteAddressAsync()
        {
            ...
            if (models.Count() > 0)
            {
                ...
                if ((await MessageBox.Show("确认删除选中的地址项吗？".GetLanguageValue(App.LanguageOperate), "温馨提示".GetLanguageValue(App.LanguageOperate), Windows.Controls.@enum.MessageBoxButton.OKCancel, Windows.Controls.@enum.MessageBoxImage.Question)).ToBool())
                {
                    ...
                    if (failStatus > 0)
                    {
                        await MessageBox.Show($"{"存在".GetLanguageValue(App.LanguageOperate)}“{failStatus}”{"个点位删除失败".GetLanguageValue(App.LanguageOperate)}", "温馨提示".GetLanguageValue(App.LanguageOperate), Windows.Controls.@enum.MessageBoxButton.OK, Windows.Controls.@enum.MessageBoxImage.Error);
                    }
                    else
                    {
